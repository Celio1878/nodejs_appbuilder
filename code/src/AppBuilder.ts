...
import { AppBuilderReturnType, BuildArgs, Middleware, Module } from "./utils/ApiTypes";
...
export function AppBuilder(modules: Module[] = []): AppBuilderReturnType {
	...
	return {
		...
		build(args: Partial<BuildArgs>) {
			...
			config_middlewares.forEach((config_middleware) => app.use("*", config_middleware));
			...
			modules.forEach((module) =>
				add_module_endpoints_to_app(app, module, modules_map, default_props.base_url)
			);
...
function default_args(args: Partial<BuildArgs>): BuildArgs {
	const DEFAULT_CONFIG: BuildArgs = {
		...
		error_handler: (error: any, req: Request, res: Response) => reply_error(res, error),
	};
...
function add_module_endpoints_to_app(
	...
	(module.endpoints || []).forEach(({ method, endpoint, middlewares }) => {
		...
		const module_base_url_sanitized = remove_trim_slashes(module.base_url || "");
	...
	module.children?.forEach((child_module) =>
		add_module_endpoints_to_app(app, child_module, modules_map, `${base_url}/${module.base_url}`)
	);
